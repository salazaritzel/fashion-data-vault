...
const FormSchema = z.object({
  ...
  amount: z.coerce
    .number() // number is set to coerce (change) from a string to a number while also validating its type
...
export async function createInvoice(prevState: State, formData: FormData) {
  ...
  revalidatePath('/dashboard/invoices'); // Once the database has been updated, the /dashboard/invoices 
                                         // path will be revalidated, and fresh data will be fetched from the server.
  redirect('/dashboard/invoices');
}

// ADDED THIS BELOW UNSURE
